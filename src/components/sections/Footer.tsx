...
export default function Footer({ config }: any) {
  ...
  return (
    <footer className="bg-client-text-dark text-white/60 pt-16 pb-8">
      <div className="max-w-[1200px] mx-auto px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          {/* Brand */}
          <div>
            <div className="font-display text-2xl text-white mb-3">{config.nomeEmpresa || 'Padom Seguros'}</div>
            <p className="text-[0.85rem] leading-relaxed">Há 29 anos oferecendo consultoria personalizada em consórcio de veículos e imóveis. Zona Norte de São Paulo.</p>
            {config.endereco && <p className="text-[0.85rem] mt-3">{config.endereco}</p>}
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              ...
              {config.email && <li><a href={`mailto:${config.email}`} className="text-sm hover:text-client-gold transition-colors">{config.email}</a></li>}
            </ul>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6">
            ...
            <button onClick={scrollToForm} className="bg-client-gold text-client-primary px-5 py-2.5 rounded-full font-bold text-sm tracking-wide hover:bg-white transition-all duration-300">
              Solicitar contato
            </button>
          </div>
        </div>
        ...
        {/* Bottom */}
        <div className="border-t border-white/[0.08] pt-6 flex flex-col md:flex-row justify-between items-center gap-4 text-xs">
          <span>{config.copyright}</span>
          <div className="flex gap-4">
            {config.redesSociais && Object.entries(config.redesSociais).map(([rede, url]: any) => (
              <a key={rede} href={url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 bg-white/[0.06] rounded-full flex items-center justify-center hover:bg-client-gold hover:text-client-primary transition-all duration-300">
                {rede === 'instagram' && (
